    ...
    <div class="addPaperRight" >
      ...
      <el-form
        ...
        <el-form-item prop="paperType">
          <el-select v-model="paperMessage.paperType" placeholder="试卷类型" disabled>
            <el-option
...
<script>
  ...
  import { URL_GET, URL_POST, URL_PUT, TIKA_PAPER, TIKA_TIKU, TIKA_SYSTEM } from 'api/config'
  ...
  export default {
    ...
    methods: {
      ...
      _getQuestionLists(data) {//获取试题列表 分页信息列表val
        ...
        let getData = {
          url: "/v1/itembank/ItembankQuestionInfo/bigAndSmallPage?subjectCode="+this.val+'&current='+ this.current + "&size=" + this.size+ "&typeCode=" + this.typeCode + "&questionProp="+this.questionProp+"&agencyCode=" + sessionStorage.getItem("agencyCode"),
          value: TIKA_TIKU
        }
      ...
      _getAllSubject() {//获取所有科目
        let getData = {
          // url: '/v1/itembank/itembankSubject/list',
      ...
      submit(){
        ...
        this.modelDatas.forEach((res,index)=>{
          this.modelDatas[index].sumScore=0
          this.modelDatas[index].children.forEach(resChild=>{
            this.modelDatas[index].sumScore+=parseInt(resChild.source)
          })
        })
        let format = {}
